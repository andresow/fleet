...
class PolizasReport(models.TransientModel):
    ...
    def action_print_polizas_report(self):
        ...
        end_days = start_date + relativedelta(months=+1) - timedelta(days=1)
        end_date = fields.Datetime.from_string(str(self.fecha_ano) + '-' + str(self.fecha_mes) + '-' + str(end_days.day))
        domain.append(('date','>=',start_date))
        domain.append(('date','<=',end_date))

        journal_entries = self.env['account.move'].search(domain)
        company = self.env.company #self.env.user.company_id
